...
class BeersController < ApplicationController
  ...
  def index
    if params[:search].present?
      @beers = Beer.global_search(params[:search]).paginate(page: params[:page], per_page: 24).order("name ASC")
      @articles = Article.all
      @typical_beers = TypicalBeer.all
      @fancy = Fancy.new
      @rating = Rating.new
    elsif params[:color_search].present?
      @beers = Beer.color_search(params[:color_search]).paginate(page: params[:page], per_page: 24).order("name ASC")
      @articles = Article.all
      @typical_beers = TypicalBeer.all
      @fancy = Fancy.new
      @rating = Rating.new
    else
      @beers = Beer.all.paginate(page: params[:page], per_page: 24).order("name ASC")
      @articles = Article.all
      @typical_beers = TypicalBeer.all
      @fancy = Fancy.new
      @rating = Rating.new
